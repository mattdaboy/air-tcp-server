...
//        dispatchEvent(new CustomEvent(CustomEvent.IO_ERROR));
    ...
    public function load(url:String):void {
        ...
        _image_loader.alpha = 0;
        _image_loader.x = 0;
        _image_loader.y = 0;
        _image_loader.scaleX = 1;
        _image_loader.scaleY = 1;
    ...
    public function unload():void {
        if (_image_loader_added) {
            Tools.safeRemoveChild(_container, _image_loader);
            _image_loader_added = false;
        }
    ...
    private function completeHandler(e:Event):void {
        ...
        _bitmap_bis.alpha = 0;
        _bitmap_bis.x = _image_loader.x;
        _bitmap_bis.y = _image_loader.y;
        _bitmap_bis.width = _image_loader.width;
        _bitmap_bis.height = _image_loader.height;
    ...
    private function loadEnd():void {
        ...
        addChild(_container);
        _container.addChild(_image_loader);
    ...
    public function imageLoaderScale(zoom:String):void {
        ...
        if (zoom != NONE) {
            ...
            _image_loader.x = imgloader_finalx;
            _image_loader.y = imgloader_finaly;
            _image_loader.scaleX = imgloader_finalscalex;
            _image_loader.scaleY = imgloader_finalscaley;
            ...
            if (_animation) {
                TweenLite.to(_image_loader, Const.ANIM_DURATION, {alpha: 1, ease: Power2.easeOut});
                TweenLite.to(_bitmap_bis, Const.ANIM_DURATION, {alpha: .1, ease: Power2.easeOut});
            } else {
                _image_loader.alpha = 1;
                _bitmap_bis.alpha = .1;
            }

            if (ratio == "portrait") {
                _bounds = new Rectangle(0, imgloader_finaly * 2, 0, -imgloader_finaly * 2);
            } else {
                _bounds = new Rectangle(imgloader_finalx * 2, 0, -imgloader_finalx * 2, 0);
            }

            _dummy.x = imgloader_finalx;
            _dummy.y = imgloader_finaly;
        }
    }
    ...
    public function setDragable(dragable:Boolean):void {
        if (dragable) {
            if (_zoom == FILL) {
                addDragListeners();
                addChild(_bitmap_bis);
            }
        } else {
            removeDragListeners();
            if (_bitmap_bis.stage != null)
                removeChild(_bitmap_bis);
        }
    }

    private function addDragListeners():void {
        if (!_image_loader.hasEventListener(MouseEvent.MOUSE_DOWN))
            _image_loader.addEventListener(MouseEvent.MOUSE_DOWN, mouseDownHandler);
    }
    ...
    private function removeDragListeners():void {
        if (_image_loader.hasEventListener(MouseEvent.MOUSE_DOWN)) _image_loader.removeEventListener(MouseEvent.MOUSE_DOWN, mouseDownHandler);
        if (_dummy.hasEventListener(Event.ENTER_FRAME)) _dummy.removeEventListener(Event.ENTER_FRAME, dummyEnterframeHandler);
    }

    private function dummyEnterframeHandler(event:Event):void {
        _image_loader.x = _bitmap_bis.x = _dummy.x;
        _image_loader.y = _bitmap_bis.y = _dummy.y;
    }
