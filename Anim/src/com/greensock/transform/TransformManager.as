...
public class TransformManager extends EventDispatcher {
        ...
        if (_enabled && _selectedItems.length != 0 && _parent != null && _parent.contains(_dummyBox) && _parent.contains(_selection)) {
            ...
            for (i = _items.length - 1; i > -1; i--) {
                if (_items[i].targetObject.parent == _parent) {
                    curDepths.push({
                        depth: (useElement ? Object(_parent).getElementIndex(_items[i].targetObject) : _parent.getChildIndex(_items[i].targetObject)),
                        item: _items[i]
                    });
                }
            }
        ...
        if (!_initted) {
            ...
            if (_selectedItems.length == 1) {
                ...
                if (ti.manualBoundsOffset != null) {
                    ...
                    // david 修改处理 textarea 选中框位置不准的问题
                    _dummyBox.graphics.drawRect(t.x, t.y, t.width, t.height);
                    // r = t.getBounds(t);
    ...
    private function renderSelection():void { // Only makes the selection handles and edges match where the _dummyBox is
        if (_initted) {
            ...
            if (_lineThickness) {
                _selection.graphics.lineStyle(_lineThickness, _lineColor, 0, false, "none");
            }
    ...
    public function addSelectionBoxElement(element:DisplayObject, alignment:String = "topRight", xOffset:Number = 0, yOffset:Number = 0):void {
        ...
        _selectionElements.push({
            element: element,
            alignment: alignment,
            angle: Math.atan2(yOffset, xOffset),
            length: Math.sqrt(xOffset * xOffset + yOffset * yOffset)
        });
        if (_selection != null) {
    ...
    public function getSelectionBoxElement():DisplayObject {
        return _selectionElements[0]["element"];
    }
    ...
    private function getSelectionRect():Rectangle {
        ...
        var t:DisplayObject = _selectedItems[i].targetObject;
        var b:Rectangle = new Rectangle(t.x, t.y, t.width, t.height);

        // var b:Rectangle = _selectedItems[i].targetObject.getBounds(_parent);
        while (i--) {
            t = _selectedItems[i].targetObject;
            var c:Rectangle = new Rectangle(t.x, t.y, t.width, t.height);
    ...
    private function createHandle($name:String, $type:String, $cursorRotation:Number, $flipRotation:Number = 0, $oppositeName:String = null):Object {
        ...
        h.name = $name;
        if (h.name == "br") {
            var btnResize:BulleBtnResizeView = new BulleBtnResizeView();
            h.addChild(btnResize);
            btnResize.x = btnResize.y = -40;
            // color
            var color:uint = DataManager.parametersData.ihm.couleur;
            var colorTransform:ColorTransform = new ColorTransform();
            colorTransform.color = color;
            btnResize.$fond.transform.colorTransform = colorTransform;
        }
    ...
    private function redrawHandles():void {
        ...
        for (i = 0; i < _handles.length; i++) {
            ...
            s.graphics.clear();
            if (_lineThickness) {
                s.graphics.lineStyle(_lineThickness, _lineColor, 0, false, "none");
            }
    ...
    private function onPressMove($e:Event):void {
        if (!_selLockPosition) {
            ...
            _trackingInfo = {
                moved: false,
                offsetX: _parent.mouseX - _dummyBox.x,
                offsetY: _parent.mouseY - _dummyBox.y,
                x: _dummyBox.x,
                y: _dummyBox.y,
    ...
    public function moveSelection($x:Number, $y:Number, $dispatchEvents:Boolean = true):void {
        ...
        if (!_selLockPosition) {
            ...
            // david 修改textarea 移动控制边界不准问题
            // new Rectangle(_dummyBox.x,_dummyBox.y,_dummyBox.width,_dummyBox.height)
            var dummyBoxRect:Rectangle = _dummyBox.getBounds(_dummyBox);
            // dummyBoxRect.x=_dummyBox.x;
            // dummyBoxRect.y=_dummyBox.y;
            // trace("dummyBox:");
            // trace(_selMinBounds);
            // var dummyBoxRect:Rectangle=getSelectionRect();

            if (_selMinBounds != null && !_selMinBounds.containsRect(dummyBoxRect)) {
